...
with open(os.path.join(BASE, "ride_policy.json"), "r") as f:
    data = json.load(f)
...
def dot(vec, matrix):
    # matrix: list of rows OR columns (we handle column format below)
    return [
        sum(v * w for v, w in zip(vec, col))
